  ...
  return (
    <section className="card" aria-labelledby="form-h">
      ...
      <div className="grid">
        ...
        <label>Car fuel
          <select value={value.car_fuel}
            onChange={(e) => onChange({ ...value, car_fuel: e.target.value as TransportMode })}>
            {FUELS.map((f) => <option key={f} value={f}>{f.replace("_", " ")}</option>)}
          </select>
        </label>
        <label>Rail km / week
        ...
        <label>Flight hours / year
          <input type="number" min={0} value={value.flight_hours_year} onChange={num("flight_hours_year")} />
        </label>
        <label>Diet
          <select value={value.diet}
        ...
        <label>Electricity kWh / month
          <input type="number" min={0} value={value.electricity_kwh_month} onChange={num("electricity_kwh_month")} />
        </label>
        <label>Shopping ₹ / month
          <input type="number" min={0} value={value.shopping_inr_month} onChange={num("shopping_inr_month")} />
        </label>
      </div>
      <p><button type="button" onClick={onSubmit}>Calculate footprint</button></p>
    </section>
  );
}
